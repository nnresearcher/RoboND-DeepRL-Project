...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		*/
		//first
		//if ((strcmp(contacts->contact(i).collision1().c_str(), COLLISION_ITEM) == 0) && !testAnimation)
		//second
...
bool ArmPlugin::updateAgent()
{
	...
	/
	*/
	const int vel_index=action/2;
	float velocity = vel[vel_index] + actionVelDelta * ((action % 2 == 0) ? 1.0f : -1.0f); // TODO - Set joint velocity based on whether action is even or odd.
	...
	/
	*/
	const int joint_index = action / 2;
	float joint = ref[joint_index] + actionJointDelta * ((action % 2 == 0) ? 1.0f : -1.0f); // TODO - Set joint position based on whether action is even or odd.
		...
		*/ 
		...
		if(!(gripBBox.min.z <= groundContact))
		{
			...
			if( episodeFrames > 1 )
			{
				...
				//rewardHistory = avgGoalDelta+5*exp(-distGoal);//first
           //rewardHistory = 10*exp(-distGoal);//second
				//rewardHistory= 2.0f*exp(-distGoal); //second
